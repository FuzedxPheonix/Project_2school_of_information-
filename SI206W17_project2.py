...
searched_tweets = api.search(q=query,parser=tweepy.parsers.JSONParser())

verison_query = "McGill University"

searched_McGill_University =  api.search(q=verison_query,parser=tweepy.parsers.JSONParser())

list_of_items = []

first_base_url = "https://www.si.umich.edu/directory?field_person_firstname_value=&field_person_lastname_value=&rid=All"

base_url_usmi_direct = "https://www.si.umich.edu/directory?field_person_firstname_value=&field_person_lastna%20me_value=&rid=All&page="
...
try:
	...
	with open('206project2_caching.json', 'w') as newFile:
		cache_dict["umsi_directory_data"] = list_of_items
		cache_dict["twitter_University of Michigan"] = searched_tweets
		cache_dict["twitter_McGill University"] = searched_McGill_University
		new_r_one = requests.get(first_base_url, headers={'User-Agent': 'SI_CLASS'})
		temp_html = new_r_one.text
		list_of_items.append(temp_html)

		i = 1
		total = 12
		while i < total:

			temp_url = base_url_usmi_direct + str(i)
			new_r = requests.get(temp_url, headers={'User-Agent': 'SI_CLASS'})
			html_stuff = new_r.text
			list_of_items.append(html_stuff)
			i +=1

		json.dump(cache_dict, newFile)
		newFile.close()	

	with open('206project2_caching.json') as data_file:
		CACHE_DICTION = json.load(data_file)


except:
	CACHE_DICTION = "206project2_caching.json"
...
def find_urls(some_string):
	regex = r"((http|https)://+.[A-za-z0-9.]+\w)"
	no = re.findall(regex, some_string, re.MULTILINE)
	lst_return = []
	for x in no:
		(one,two)  = x
		lst_return.append(one)
...
def get_umsi_data():
	temp_lst = []
	for item in CACHE_DICTION["umsi_directory_data"]:
...
all_html_data = CACHE_DICTION["umsi_directory_data"]

umsi_titles = {}

for items in all_html_data:
	soup = BeautifulSoup(items, "html.parser")
	title_positions = soup.find_all("div", {"class":"field-name-field-person-titles"})

	temp_lst_names = []
	for div in soup.find_all(property="dc:title"):

	    temp_lst_names.append(div.get_text())

	temp_lst_titles = []

	for divv in title_positions:
		temp_lst_titles.append(divv.get_text())

	counter = 0
	for item in temp_lst_names:
		umsi_titles[item] = temp_lst_titles[counter]
		counter+= 1
...
def get_five_tweets(random_link):
	searched_item = api.search(q=random_link,parser=tweepy.parsers.JSONParser())
	seach_query = searched_item["search_metadata"]["query"]
	...
	try:
		for item in CACHE_DICTION:
			if seach_query == str(CACHE_DICTION[item]["search_metadata"]["query"]):
				print(CACHE_DICTION[item]["search_metadata"]["query"])
				count_temp = 0

				while count_temp < 5:
					print(CACHE_DICTION[item]["statuses"][count_temp]["text"])
					lst_of_tweets.append(CACHE_DICTION[item]["statuses"][count_temp]["text"])
					count_temp+= 1

				return lst_of_tweets
		...
		print("ERROR")
		new_str = "twitter_" + str(seach_query)
		update_search = api.search(q=random_link, limit = 5, parser=tweepy.parsers.JSONParser())

		with open("206project2_caching.json") as oldFile:
			new_data = json.load(oldFile)

		temp_dict = {}

		temp_dict[new_str] = update_search

		new_data.update(temp_dict)

		with open('206project2_caching.json', 'w') as f:
			json.dump(new_data, f)

		count_val = 0
		while count_val < 5:
			lst_of_tweets.append(update_search["statuses"][count_val]["text"])
			count_val+= 1
